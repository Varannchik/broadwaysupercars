$(window).load(function(){
    ...
    var size_specials_slider=Math.floor($(".slider_duble").width()/400);
    size_specials_slider=(size_specials_slider<1) ? 1 : size_specials_slider;
    $(".slider_duble li").width(Math.floor($(".slider_duble").width()/size_specials_slider));
    if($(".slider_duble li").size()>2){
        $(".slider_duble").touchCarousel({
            itemsPerMove: 1,
            pagingNav: true,
            scrollbar: false,
